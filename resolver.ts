...
export const resolvers = {
  ...
  Mutation: {
    ...
    async updateUser(
      _,
      { id, email, role, team, userType, fullName, valuePerHour }
    ) {
      const updated = (
    ...
    async setTimeStamp(_, { id, lastTimeStamp }) {
      const updated = (
        await User.updateOne({ _id: id }, { lastTimeStamp: lastTimeStamp })
      ).modifiedCount;
      return updated;
    },

    async giveUserTask(_, { userID, taskID }) {
      //Insert a new task in User.tasks and a new assign in Task.assigns
    ...
    async updateTeam(_, { id, name, RHManager }) {
      const updated = (
        await Team.updateOne({ _id: id }, { name: name, RHManager: RHManager })
      ).modifiedCount;
    ...
    async newMemberTeam(_, { teamID, userID }) {
      //Insert a new User in Team.members and update Team.team
    ...
    async defineTechLead(_, { teamID, techID }) {
      //Verify if the User is a team's member before set him as TechLead
      let isMember = false;
      let updated;
      ...
      response.members.forEach((member) => {
        if (newTech == member.memberID) isMember = true;
      });
      if (isMember)
        updated = (await Team.updateOne({ _id: teamID }, { techLead: newTech }))
          .modifiedCount;
      else updated = false;
      return updated;
    },
    ...
    async updateTask(_, { id, name, description, status, githubUrl }) {
      const updated = (
        await Task.updateOne(
          { _id: id },
          {
    ...
    async setTimeHoursWorked(_, { id, hoursWorked }) {
      const updated = (
        await User.updateOne({ _id: id }, { hoursWorked: hoursWorked })
      ).modifiedCount;
      return updated;
    },
  },
